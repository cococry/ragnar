...
truecolorwindow(state_t* s, area_t a, uint32_t bw) {
  ...
  logmsg(LogLevelTrace, "created window with true-color colormap (p: %ix%i s: %ix%i).",
         a.pos.x, a.pos.y, a.size.x, a.size.y);
...
evmaprequest(state_t* s, xcb_generic_event_t* ev) {
  ...
  client_t* cl = addclient(s, map_ev->window);
  ...
  if(s->curlayout != LayoutFloating) {
    ...
    for(client_t* cl = s->clients; cl != NULL; cl = cl->next) {
      if(clientonscreen(s, cl, s->monfocus) && cl->floating) {
        raiseclient(s, cl);
      }
    }
  }
