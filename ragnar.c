...
char* get_window_title(Display *display, Window window) {
  // Try to get the window name using XFetchName()
  char *windowName = NULL;
  if (XFetchName(display, window, &windowName))
    return windowName;

  // If XFetchName() fails, try to get the window title using XGetWMName()
  XTextProperty windowNameProperty;
  if (XGetWMName(display, window, &windowNameProperty) != 0) {
    int count;
    char **list;
    if (Xutf8TextPropertyToTextList(display, &windowNameProperty, &list, &count) >= Success && count > 0) {
...
void draw_bar() {
  ...
  float padding = 25.0f;
  uint32_t xoffset = Monitors[wm.bar_monitor].width - BAR_PADDING_X - sizex - padding;
  {
    for(uint32_t i = 0; i < BAR_COMMANDS_COUNT; i++) {
      // Passing the output of the given bar command to the text 
      get_cmd_output(BarCommands[i].cmd, BarCommands[i]._text);
...
int32_t draw_bar_desktop_label() {
  ...
  char* focused_title = get_window_title(wm.display, wm.client_windows[wm.focused_client].win);
