 ...
 * */
void
ignoreenterlayout(state_t* s) {
  s->ignore_enter_layout = true;
  xcb_query_pointer_reply_t* reply = xcb_query_pointer_reply(
      s->con, xcb_query_pointer(s->con, s->root), NULL);
  if(reply) {
    s->ignore_enter_pos = (v2_t){reply->root_x, reply->root_y};
    free(reply);
  }
}
...
eventernotify(state_t* s, xcb_generic_event_t* ev) {
  ...
  if(s->ignore_enter_layout) {
    // an enter at the stored position comes from windows shifting
    // under a resting pointer, anywhere else is real movement
    if(enter_ev->root_x == (int16_t)s->ignore_enter_pos.x &&
       enter_ev->root_y == (int16_t)s->ignore_enter_pos.y) {
      return;
    }
...
evmotionnotify(state_t* s, xcb_generic_event_t* ev) {
  ...
  if(s->ignore_enter_layout) {
    s->ignore_enter_layout = false;
    // the swallowed enter is never resent, refocus what is under the pointer
    focus_top_client_under_cursor(s, s->con, s->root);
  }
